// Author: Chloe Huang, Hawaii Pacific University
// partially based on code from Jiru Xu and Dr. Curtis Powley
...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (N != 0) { // loop thru problems
            ...
            for (int j = 0; j < N; j++) {
                vertices[j] = new Vertex(j); // create vertices for the new slots
            }
            ...
            dijkstra(vertices[s], vertices[s]); //s or d?
            
            for (int i = 0; i < numQueries; i++) {
                int d = in.nextInt(); // destination node (need to be in a loop)
    ...
    public static class Vertex implements Comparable<Vertex> {
        ...
        // public double minDis = Double.POSITIVE_INFINITY; // use if edge weights are
        // not integers
        ...
        public int compareTo(Vertex other) {
            // return Double.compare(this.minDis, other.minDis); // use if edges are not
            // integers
